...
from earthcatalog.transform import write_geoparquet as _write_gp
...
class TestConsolidatePartition:
    ...
    def test_merges_all_small_files(self):
        ...
        files_info = [
            {
                "path": f"grid_partition={cell}/year={year}/part_{idx:06d}.parquet",
                "name": f"part_{idx:06d}.parquet",
                "index": idx,
                "size": 0,
            }
            for idx in range(3)
        ...
        result = consolidate_partition(
            store, cell, year, files_info, threshold=2, dry_run=False, compact_rows=10
        )
        ...
        consolidated = pq.ParquetFile(
            io.BytesIO(
                bytes(
                    obstore.get(
                        store, f"grid_partition={cell}/year={year}/part_000000.parquet"
                    ).bytes()
                )
            )
        ).read()
    ...
    def test_keeps_full_files_intact(self):
        ...
        files_info = [
            {
                "path": f"grid_partition={cell}/year={year}/part_{i:06d}.parquet",
                "name": f"part_{i:06d}.parquet",
                "index": i,
                "size": 0,
            }
            for i in range(2)
        ...
        small = pq.ParquetFile(
            io.BytesIO(
                bytes(
                    obstore.get(
                        store, f"grid_partition={cell}/year={year}/part_000001.parquet"
                    ).bytes()
                )
            )
        ).read()
    ...
    def test_consolidates_trailing_after_full(self):
        ...
        result = consolidate_partition(
            store, cell, year, files_info, threshold=2, dry_run=False, compact_rows=100
        )
    ...
    def test_dry_run_makes_no_changes(self):
        ...
        result = consolidate_partition(
            store, cell, year, files_info, threshold=2, dry_run=True, compact_rows=10
        )
        assert result["merged_files"] == 2
    ...
    def test_below_threshold_skips(self):
        ...
        files_info = [
            {
                "path": f"grid_partition={cell}/year={year}/part_000000.parquet",
                "name": "part_000000.parquet",
                "index": 0,
                "size": 0,
            }
        ]
