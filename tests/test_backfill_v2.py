"""
Tests for earthcatalog.pipelines.backfill_v2.
...
class TestWriteChunks:
    def test_writes_chunk_parquets(self, tmp_path):
        from earthcatalog.pipelines.backfill_v2 import write_chunks
...
class TestFetchItemAsync:
    def test_fetches_from_real_memory_store(self):
        ...
        from earthcatalog.pipelines.backfill_v2 import _fetch_item_async
    ...
    def test_403_raises_immediately(self):
        ...
        with patch("earthcatalog.pipelines.backfill_v2.obstore.get_async", side_effect=_raise_403):
...
class TestFetchAllAsync:
    def test_fetches_multiple_items(self):
        ...
        from earthcatalog.pipelines.backfill_v2 import _fetch_all_async
...
class TestWriteNdjson:
    def test_writes_ndjson_to_store(self):
        from earthcatalog.pipelines.backfill_v2 import _write_ndjson
...
class TestListCompletedChunkIds:
    def test_returns_empty_when_no_markers(self):
        from earthcatalog.pipelines.backfill_v2 import _list_completed_chunk_ids
...
class TestIngestChunk:
    def test_reads_chunk_and_writes_ndjson(self, tmp_path):
        ...
        from earthcatalog.pipelines.backfill_v2 import ingest_chunk
        ...
        with patch("earthcatalog.pipelines.backfill_v2._fetch_item_async", side_effect=_mock):
            report = ingest_chunk(
...
class TestReadNdjsonFiles:
    def test_reads_multiple_files(self):
        from earthcatalog.pipelines.backfill_v2 import _read_ndjson_files
...
class TestDedupItems:
    def test_deduplicates_by_id(self):
        from earthcatalog.pipelines.backfill_v2 import _dedup_items
...
class TestCompactCellYear:
    def test_reads_ndjson_and_writes_parquet(self, tmp_path):
        from earthcatalog.pipelines.backfill_v2 import compact_cell_year
...
class TestCompactCellYearS3:
    def test_writes_parquet_to_store(self):
        from earthcatalog.pipelines.backfill_v2 import compact_cell_year_s3
...
class TestRegisterAndCleanup:
    def test_registers_files_and_cleans_up(self, tmp_path):
        from earthcatalog.pipelines.backfill_v2 import register_and_cleanup
...
class TestRunBackfillV2:
    ...
    def test_end_to_end_with_mock_fetcher(self, tmp_path):
        ...
        with (
            patch("earthcatalog.pipelines.backfill_v2._fetch_item_async", side_effect=_mock_fetch),
...
class TestNextPartIndexLocal:
    def test_returns_zero_when_no_files(self, tmp_path):
        from earthcatalog.pipelines.backfill_v2 import _next_part_index_local
...
class TestNextPartIndexS3:
    def test_returns_zero_when_empty(self):
        from earthcatalog.pipelines.backfill_v2 import _next_part_index_s3
...
class TestCompactCellYearDelta:
    def test_writes_new_files_without_overwriting(self, tmp_path):
        from earthcatalog.pipelines.backfill_v2 import compact_cell_year_delta
...
class TestCompactCellYearDeltaS3:
    def test_writes_new_files_with_offset(self):
        from earthcatalog.pipelines.backfill_v2 import compact_cell_year_delta_s3
...
class TestRegisterDelta:
    def test_adds_files_to_existing_table(self, tmp_path):
        from earthcatalog.pipelines.backfill_v2 import register_delta
        ...
        from earthcatalog.core.catalog import FULL_NAME, ICEBERG_SCHEMA, NAMESPACE, PARTITION_SPEC, open_catalog
...
class TestRunBackfillV2Delta:
    ...
    def test_delta_does_not_overwrite_existing_parquets(self, tmp_path):
        ...
        with (
            patch("earthcatalog.pipelines.backfill_v2._fetch_item_async", side_effect=_mock_fetch_all),
            dask.config.set(scheduler="synchronous"),
        ):
            run_backfill_v2(
